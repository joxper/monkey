package com.apps.jorge.monkeybisfire.data;
...
public class Constant {
    ...
    public static List<Friendb> getFriendsData(Context ctx)  {
        List<Friendb> items = new ArrayList<>();
        ...
        for (int i = 0; i < s_arr.length; i++) {
            Friendb fr = new Friendb(i, s_arr[i], drw_arr.getResourceId(i, -1));
            items.add(fr);
        }
        return items;
    }
    ...
    public static List<Chat> getChatsData(Context ctx)  {
        ...
        for (int i = 0; i < 10; i++) {
            items.add(new Chat(i, s_date[i], true, new Friendb(s_arr[i+5], drw_arr.getResourceId(i+5, -1)), s_cht[i]));
        }
        return items;
    }
    ...
    private static ArrayList<Friendb> friendSubList(Context ctx, int start, int end){
        ArrayList<Friendb> friends = new ArrayList<>();
        friends.addAll(getFriendsData(ctx));
        ArrayList<Friendb> friends_ = new ArrayList<>();
    ...
    public static List<ChatsDetails> getChatDetailsData(Context ctx, Friendb friend)  {
    ...
    public static List<GroupDetails> getGroupDetailsData(Context ctx)  {
        ...
        List<Friendb> friends = getFriendsData(ctx);
